...
// ── Timing config ──────────────────────────────────────────────
const START_DELAY    = 4000;   // wait before first popup
const SHOW_DURATION   = 5000;   // how long each popup stays visible
const GAP_DURATION    = 5000;   // gap between one popup hiding and next showing
const MAX_PURCHASES   = 15;     // only cycle through the last 15 orders
...
const RecentPurchasePopup = () => {
  ...
  useEffect(() => {
    fetchRecentPurchases().then(data => {
      // Keep only the most recent MAX_PURCHASES orders, and drop
      // anything older than 7 days (timeAgo returns null for those).
      const valid = (data || [])
        .filter(p => timeAgo(p.created_at) !== null)
  ...
  useEffect(() => {
    ...
    const showNext = () => {
      ...
      if (!timeStr) {
        indexRef.current++;
        nextTimer.current = setTimeout(showNext, GAP_DURATION);
        return;
      }
    ...
    nextTimer.current = setTimeout(showNext, START_DELAY);
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      ...
      // Resume cycling after the normal gap
        ...
        if (timeStr) {
          ...
          hideTimer.current = setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              setLeaving(false);
              indexRef.current++;
              nextTimer.current = setTimeout(() => {}, GAP_DURATION);
            }, 380);
          }, SHOW_DURATION);
        }
      }, GAP_DURATION);
