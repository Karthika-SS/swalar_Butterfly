...
.btn-cancel-delete:hover { border-color: var(--text-muted); color: var(--text); }
  ...
  <div className="delete-modal-overlay" onClick={onCancel}>
    <div className="delete-modal" onClick={e => e.stopPropagation()}>
      ...
      <div className="delete-modal-actions">
        <button className="btn-cancel-delete" onClick={onCancel} disabled={deleting}>
          Cancel
        </button>
        <button className="btn-confirm-delete" onClick={onConfirm} disabled={deleting}>
...
const OrderManagement = () => {
  ...
  const downloadShippingSlip = (order) => {
    ...
    const addressLines = doc.splitTextToSize(order.customer_address || '', 160);
    ...
    order.items?.forEach((item, index) => {
      ...
      y += 8;
      doc.text(`Size: ${item.size || 'No Size'} - Qty: ${item.quantity} - Price: Rs.${item.price}`, 30, y);
      y += 12;
